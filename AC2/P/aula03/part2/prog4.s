...
wait:
    ...
    syscall
    blt     $v0,    60000000,           wait    # 3 Hz

    lw      $t4,    PORTB($t0)                  # READ PORTB
    andi    $t4,    $t4,                0x0002  # Isolate bit 1

    bne     $t4,    $0,                 else    # if RB1 == 0 shift right
    srl     $t2,    $t2,                1
    j       endif

else:                                           # if RB1 != 0 shift left
    sll     $t2,    $t2,                1

endif:
    bgtu    $t2,    0x0008,             if1     # 1000 to 0001 when sll
    beq     $t2,    $0,                 if2     # 0001 to 1000 when srl
    j       while
